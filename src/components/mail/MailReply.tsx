        ...
        >
            <Box $display='flex' $gap={15} $alignItems="flex-start">
                <Box $width={40} $minWidth={40} $height={40} $minHeight={40}>
                    <Avatar type='url' url={employee.data?.avatarUrl || defaultAvatar} />
                </Box>
                <Box $color="#5E5E5E" $lineHeight={1.5}>
                    <Text><b>{employee.data?.fullName}</b> <Text $display='inline' $fontSize={".7rem"}>{`<${employee.data?.accountEmail}>`}</Text></Text>
                    <Text $fontSize={"0.7rem"}>Đã trả lời lúc {new Date(mailReply.repliedAt).toLocaleString("vi-VN", { timeStyle: "short", dateStyle: "medium" })}</Text>
                    <Box $color='#000' $marginTop={20} $borderLeft={"1px solid #ccc"} $paddingLeft={10}>
                        <Box
                            $display="flex"
                            $alignItems="center"
                            $gap={10}
                        ...
                        {React.createElement("div", { dangerouslySetInnerHTML: { __html: mailReply.content } })}
                    </Box>
                </Box>
            </Box>
        </Box>

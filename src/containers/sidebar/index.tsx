...
const Sidebar = () => {
    ...
    const { mailBoxList, currentMailBox } = state.mail;
        ...
        if (type === "chat" && currentTag) {
            return (
                <Box $padding={"25px 0"} $borderBottom={"1px solid #ccc"}>
                    <Box
                        $fontSize={".8rem"} $padding={"10px 0"} $color="#000"
    ...
    return (
        <section className="sidebar-container">
            <SearchBar
                placeholder={sidebarState?.type === "chat" ? "Tìm kiếm tin nhắn..." : "Tìm kiếm email..."}
                inputValue={search}
            ...
            {sidebarState.listTags &&
                <ListTag
                    listTags={sidebarState.listTags}
                    renderListTags={renderListTags}
                />}
        </section>
    )
}

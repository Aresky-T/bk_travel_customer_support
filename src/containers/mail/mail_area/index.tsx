...
import { deleteMailBoxByIdApi } from "../../../api/mail.api"
...
const MailArea = () => {
    ...
    const { currentMail, currentMailBox } = state.mail;

    function onLeaveBox() {
        if (dispatch) {
            currentMail && dispatch({ type: "MAIL|REMOVE|CURRENT_MAIL" });
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    ...
                    if (token && currentMailBox) {
                        deleteMailBoxByIdApi(token, currentMailBox.id)
                            .then(res => {
                                if (res.data) {
                                    ...
                                    getStompClient()
                                        .then(client => {
                                            client.publish({
                                                destination: `/app/mail/load.mailboxes.of.employee/${state.employee.data?.id}`
                                            })
                                        })
                                    if (dispatch) {
                                        dispatch({ type: "MAIL|REMOVE|CURRENT_MAIL_BOX" });
    ...
    function reloadMailBox() {
        ...
        if (employee && employee.id) {
            getStompClient()
                .then(client => {
                    client.publish({
                        destination: `/app/mail/load.mailboxes.of.employee/${employee.id}`
                    })
                })
        }
    }
                    ...
                    <Box className="mail-box__header">
                        <div className="mail-box__header__customer">
                            <Box className="customer__avatar">
                                {currentMailBox.customer.avatarUrl ?
                                    <Avatar type="url" url={currentMailBox.customer.avatarUrl} />
                                    :
                                    <Avatar type="text" text={currentMailBox.customer.fullName} />
                                }
                            </Box>
                        ...
                        <Box className="mail-box__header__options"
                            $display="flex"
                            $alignItems="center"
                        ...
                        >
                            {currentMail &&
                                <Button icon={HiMiniArrowUturnLeft}
                                    onClick={() => {
                                        if (dispatch) {
                    ...
                    <Box className="mail-box__body">
                        <Box className="mail-box__body__mails cs-scroll">
                            {currentMail ?
                                <MailDetails
                                    mail={currentMail}
                                    customer={currentMailBox.customer}
                                />
                                :
                                <>
                                    {currentMailBox.mailList?.map(mail => (
